...
static char *list_of_directory[819200];
unsigned int lod_length = 0;
	...
	for (int s = 0; s < lod_length - 1; s++) {
		...
		for (int d = 0; d < lod_length - 1; d++) {
			...
			if (strcmp(list_of_directory[d], list_of_directory[d + 1]) > 0) {
				...
				//list_of_directory[d] = list_of_directory[d + 1];
				
				strcpy(list_of_directory[d + 1], t);
				//list_of_directory[d + 1] = t;
			}
		}
	}
}
...
void pp_colored(char buf[4096], int offset, char filename[4096]) {
	...
	//case S_IXUSR:  printf("\033[01;32m");break;		// executabale
	...
	while (1) {
		...
		*/
		...
		if ( ch == kb_exit ) {
			...
			if (current_directory != "/") chdir(".."); selected_file = 0;
		}
		else if ( ch == kb_child_d ) {
			//chtochild();
			if (is_directory(list_of_directory[selected_file]) == 0) {
				/* if the file is a symbolic link, change directory
				 * to the linked path. else, change directory normally
				 */

				/*
				printf("debug--key pressed, child_d");
				if (is_symlink(list_of_directory[selected_file]) == 1) {
					printf("debug--key pressed, child is symlink");
					char *linkpath = "/";
					strcat(linkpath, readlink_malloc(list_of_directory[selected_file]));
					chdir(linkpath);
					selected_file = 0;
				}
				else {	
		...
		for (int d = 0 + lod_length_to_print*(current_page - 1); d < lod_length_to_print * current_page; d++) {
			...
			if (d < lod_length) {
				...
				/*
				if (is_directory(selected_path) == 0) strcpy(b, "D");
				else strcat(b, " ");
				*/
