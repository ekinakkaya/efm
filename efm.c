...
// SET BOTH 'USE_LS_COLORS' AND 'USE_MANUAL_COLORS' TO ZERO TO DISABLE COLOR
...
#define FILE_CPY_CHUNK_SIZE    1024
...
unsigned char kb_go_to_d=0x67;       // go to dir        g
...
read_directory(char *directory)
{
	...
	if (d) {
		...
		return -1; // dir cannot be read. probably a bug.
	}

	if (lod_length != 0) return 0;
	else return 1; // dir is empty
}
...
int
yank_from_to(char so[4096], char de[4096])
{
	char cur_dir[4096];
	char des_dir[4096];

	if (is_directory(so) == 0) {
		DIR *d;
		struct dirent *dir;
		d = opendir(so);
		...
		if (d) {
			while ((dir = readdir(d)) != NULL) {
				strcpy(cur_dir, so); // /so/dir1
				strcpy(des_dir, de); // /dest/dir1copy

				// if dir does not exist, mkdir
				if (is_directory(des_dir) != 0)
					mkdir(des_dir, 0777);
				if (strcmp(dir->d_name, ".") != 0 && strcmp(dir->d_name, "..") != 0) {
					strcat(cur_dir, "/");         //   /so/dir1/
					strcat(cur_dir, dir->d_name); //   /so/dir1/file1

					strcat(des_dir, "/");         //   /dest/dir1copy/
					strcat(des_dir, dir->d_name); //   /dest/dir1copy/file1

					printf("|%s| yank to %s\n", dir->d_name, des_dir);

					// almighty gods of recursion, forever you reign...
					// we worship you, for even the grave...
					yank_from_to(cur_dir, des_dir);
				}
			}
			//getchar();
			closedir(d);
		...
		if (F_SO == NULL) {
			...
			if (F_DE == NULL) {
				...
				char ch;
				//size_t filesize;
				while ((ch = fgetc(F_SO)) != EOF) {
					fputc(ch, F_DE);
					// if 1 second is passed, update copy status
				}
			}
			fclose(F_SO);
			fclose(F_DE);
...
get_string_input(int mode, char *content, char **string_addr)
{
	...
	switch (mode) {
		...
		case 1: 
			printf("\e[%i;1f\033[K%s",ROWS, content); // move the cursor to (ROWS, 1), clear line
...
handle_ui_dirlist()
{
		...
		if (lod_length != 0)
			...
			lod_length_to_print = 0;
		//if (lod_length_to_print == 0) lod_length_to_print = 1;
		...
		if (lod_length == 0) {
			pages = 0;
		} else if (lod_length <= lod_length_to_print) {
			pages = 1;
		} else {
...
setup_terminal()
{
	printf("\e[?1049h\e[H"); /* Alternate screen buffer, disable line wrapping*/

	printf("\e[?7l");        /* Disable line wrapping */

	set_user_echo(0);        /* Set canonical mode and user echo */

	set_canonical_mode(1);   /* Enable canonical mode */
...
main(int argc, char **argv)
{
	...
	/* allocate memory for the list of directory */
	list_of_directory = malloc(LOD_INIT_SIZE * sizeof(char*));

	static int empty_dir_flag;
	...
	while (1) {
		...
		/* NAV navigation in ui */
		if (lod_length > 0) max_selection = lod_length - 1;
		else max_selection = 0;
		handle_ui_dirlist();
		...
		else if ( ch == kb_child_d && !empty_dir_flag) {
			if (is_directory(list_of_directory[selected_file]) == 0) {
				//chdir(parent_directory);
				chdir(list_of_directory[selected_file]);
				selected_file = 0;
				directory_changed = 1;
			} else if (check_file_exists(list_of_directory[selected_file]) < 0 || is_directory(list_of_directory[selected_file]) < 0) {
				selected_file = 0;
				...
				if(selected_file >= max_selection) selected_file = max_selection - 1;
			}
		}
		else if ( ch == kb_file_yank ) {
			// yank
			char source[4096];
			strcpy(source, current_directory);
			strcpy(source, "/");
			strcpy(source, list_of_directory[selected_file]);

			char *dest;
			get_string_input(1, "\e[0;30m\e[47mType destination path:\e[0m ", &dest);

			clear_scr();
			if (yank_from_to(source, dest) < 0) {
				// error
			} else {
				// directory didn't actually change, but the content did.
				directory_changed = 1;
			}
			
				yank_from_to(source, dest);
			directory_changed = 1;
		...
		// check the validity of the current directory
		//if (is_directory(current_directory) != 0) chdir("..");
		...
		//while (is_directory(parent_directory) < 0) strcat(parent_directory, "/..");
		//chdir(parent_directory);
		...
		if (directory_changed == 1) {
			int error_code = read_directory(current_directory);

			if (error_code == 1) empty_dir_flag = 1;
			else empty_dir_flag = 0;
		...
		// NAV navigation in ui
		if (lod_length > 0) current_page = (!empty_dir_flag) * (floor(selected_file / lod_length_to_print) + 1) + (empty_dir_flag) * 0;
		else current_page = 0;
